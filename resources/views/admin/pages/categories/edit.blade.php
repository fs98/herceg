...
@section('content')
  ...
  <div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-success align-middle">{{ $category->title }}</h6> 
    </div>
    <div class="card-body"> 

      <form action="{{ Route('admin.category.update', ['category' => $category->id]) }}" method="POST" id="{{ Helper::RouteCrafter('store') }}" autocomplete="off" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
          <div class="col-12">
            <div class="form-group">
              <label for="category_title" class="col-form-label">{{ __('Naziv') }}</label>
              <label for="category_title" class="label-required">{{ __('(obavezno)') }}</label>
              <input id="category_title" type="text" class="form-control" placeholder="Naziv kategorije" name="category_title" maxlength="128" value="{{ $category->title }}">
            </div>  
            ...
            <div class="form-group mt-2" id="selected_image_preview_parent">
              ...
              <img class="img-fluid" id="selected_image_preview" width="300" src="{{ $category->header_image_url }}">
            </div>
