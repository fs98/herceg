...
@section('content')
  ...
  <div class="card shadow mb-4">
    ...
    <div class="card-body"> 
      ...
      <form action="{{ Route('admin.tag.update', $tag) }}" method="POST" id="newTag" autocomplete="off" enctype="multipart/form-data">
        @method('PUT')
        <div class="form-group">
          <label for="tag_title" class="col-form-label">{{ ('Naziv') }}</label>
          <label for="tag_title" class="label-required">{{ ('(obavezno)') }}</label>
          <input id="tag_title" type="text" class="form-control" placeholder="Naziv taga" name="tag_title" maxlength="128" value="{{ old('tag_title', $tag->name) }}">
        </div>
        <div class="form-group">
          <label for="tag_color" class="col-form-label">{{ ('Boja') }}</label>
          <label for="tag_color" class="label-required">{{ ('(opcionalno)') }}</label>
          <div class="form-check">
            <input class="form-check-input" type="radio" name="tag_color" id="tagColor1" value="badge-primary" {{ old('tag_color', $tag->color) == 'badge-primary' ? 'checked' : '' }}>
            <label class="form-check-label" for="tagColor1">
              <span class="badge badge-pill badge-primary">Oznaka</span>
            </label>
          </div>
          <div class="form-check">
            <input class="form-check-input" type="radio" name="tag_color" id="tagColor2" value="badge-secondary" {{ old('tag_color', $tag->color) == 'badge-secondary' ? 'checked' : '' }}>
            <label class="form-check-label" for="tagColor2">
              <span class="badge badge-pill badge-secondary">Oznaka</span>
            </label>
          </div>
          <div class="form-check disabled">
            <input class="form-check-input" type="radio" name="tag_color" id="tagColor3" value="badge-success" {{ old('tag_color', $tag->color) == 'badge-success' ? 'checked' : '' }}>
            <label class="form-check-label" for="tagColor3">
              <span class="badge badge-pill badge-success">Oznaka</span>
            </label>
          </div>
          <div class="form-check disabled">
            <input class="form-check-input" type="radio" name="tag_color" id="tagColor3" value="badge-info" {{ old('tag_color', $tag->color) == 'badge-info' ? 'checked' : '' }}>
            <label class="form-check-label" for="tagColor3">
              <span class="badge badge-pill badge-info">Oznaka</span>
            </label>
          </div>
          <div class="form-check disabled">
            <input class="form-check-input" type="radio" name="tag_color" id="tagColor3" value="badge-warning" {{ old('tag_color', $tag->color) == 'badge-warning' ? 'checked' : '' }}>
            <label class="form-check-label" for="tagColor3">
              <span class="badge badge-pill badge-warning">Oznaka</span>
            </label>
          </div>
          <div class="form-check disabled">
            <input class="form-check-input" type="radio" name="tag_color" id="tagColor3" value="badge-danger" {{ old('tag_color', $tag->color) == 'badge-danger' ? 'checked' : '' }}>
            <label class="form-check-label" for="tagColor3">
